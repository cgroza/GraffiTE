...
exact_match.py -word_size 4 -query R.fasta -db L.fasta -outfmt 6 -strand plus | awk 'function abs(x) { return x < 0 ? -x : x } {print $0"\t"(30-($7+$8)/2)"\t"(30-($9+$10)/2)"\t"abs(30-($7+$8)/2)"\t"abs(30-($9+$10)/2)}' > blastout 2>&1

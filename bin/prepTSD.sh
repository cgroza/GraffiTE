...
bcftools view -H ${VCF} | awk -v win=${WIN} '{ if(length($4) < length($5)) {print $1"\t"($2-1-win)"\t"$2"\t"$3"__L"; print $1"\t"$2"\t"($2+1+win)"\t"$3"__R"} if (length($4) > length($5)) {print $1"\t"($2-1-win)"\t"$2"\t"$3"_L"; print $1"\t"($2+length($4))"\t"($2+1+length($4)+win)"\t"$3"_R"}}' > SV_coordinates_win.bed

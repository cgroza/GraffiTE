...
# run RepeatMasker
RepeatMasker -lib ${FASTA_LIB} -s -dir ${REPMASK_DIR} -pa $(nproc) ${FASTA_FILE}
...
awk 'NR > 3 {print $5"\t"$6-1"\t"$7"\t"$10}' ${REPMASK_OUT} | bedtools merge > merge.bed # add -1 to start to meet .bed format
...
join -13 -21 -a1 <(sort -k3,3 ${ANNOT_FILE}_1)  <(sort -k1,1 span) | sed 's/ /\t/g' | \
 ...
 awk '{if (NF == 12) {print $0"\t0\t0"} else {print $0}}' | \

...
| Parameter | Default | Effect | Source |
|---|---|---|---|
| `--assemblies` | `false` | Samplesheet of genome assemblies. Each is aligned with minimap2 (or winnowmap) and called with `svim-asm haploid`, keeping `INS` and `DEL` of 100 bp or more. | <span class="src">`nextflow.config:41`, `module/main.nf:178`</span> |
| `--longreads` | `false` | Samplesheet of unaligned long reads. Aligned, then called per sample and jointly with Sniffles2 at `--minsvlen 100`. | <span class="src">`nextflow.config:40`, `module/main.nf:105,122`</span> |
...
| `--pav` | `false` | Samplesheet of phased assemblies to call with [PAV](https://github.com/EichlerLab/pav), which runs in its own container. Keeps variants with \|SVLEN\| above 50 bp. | <span class="src">`nextflow.config:42`, `module/main.nf:161`</span> |
...
| Parameter | Default | Effect | Source |
|---|---|---|---|
| `--aligner` | `"minimap2"` | Aligner for assemblies and long reads. The only other accepted value is `"winnowmap"`. Any other value leaves `map_asm` and `map_longreads` with no script and the run fails. | <span class="src">`nextflow.config:119`, `module/main.nf:48-61`</span> |
| `--asm_divergence` | `"asm5"` | minimap2 `-x` preset for assembly alignment. Use `asm10` or `asm20` for assemblies further from the reference. Assemblies only. | <span class="src">`nextflow.config:118`, `module/main.nf:50`</span> |
| `--break_scaffolds` | `false` | Split each assembly into contigs at runs of `N` before aligning. For scaffolded input. | <span class="src">`nextflow.config:44`, `main.nf:99-101`</span> |
| `--mini_K` | `"500M"` | minimap2 and winnowmap `-K`, the number of bases loaded per batch. Larger is faster and uses more memory. | <span class="src">`nextflow.config:52`, `module/main.nf:50`</span> |
| `--stSort_m` | `"4G"` | `samtools sort -m`, memory per sort thread. Total sort memory is about `stSort_m` times `stSort_t`, on top of the aligner. | <span class="src">`nextflow.config:53`, `module/main.nf:51`</span> |
| `--stSort_t` | `4` | `samtools sort -@`, sort threads. Independent of the process `cpus`. | <span class="src">`nextflow.config:54`, `module/main.nf:51`</span> |
...
| `--human_sva_ids` | `"^SVA_[DEF]"` | SVA subfamilies. The same list gates the `Simple_repeat` records that come from VNTR-only SVA variants. | <span class="src">`nextflow.config:67`, `module/main.nf:519-520`</span> |
...
| `--human_min_svlen` | `250` (bp) | Minimum \|SVLEN\|. | <span class="src">`nextflow.config:77`, `module/main.nf:522`</span> |
...
|---|---|---|---|
| `--map_asm_threads` | `1` | `map_asm` | <span class="src">`nextflow.config:136,183`</span> |
| `--map_asm_memory` | `null` | `map_asm` | <span class="src">`nextflow.config:135,184`</span> |
| `--map_asm_time` | `"3h"` | `map_asm` | <span class="src">`nextflow.config:137,185`</span> |
| `--map_longreads_threads` | `1` | `map_longreads` | <span class="src">`nextflow.config:139,188`</span> |
| `--map_longreads_memory` | `null` | `map_longreads` | <span class="src">`nextflow.config:138,189`</span> |
| `--map_longreads_time` | `"12h"` | `map_longreads` | <span class="src">`nextflow.config:140,190`</span> |
| `--sniffles_threads` | `1` | `sniffles_sample_call`, `sniffles_population_call` | <span class="src">`nextflow.config:150,193,198`</span> |
| `--sniffles_memory` | `null` | same | <span class="src">`nextflow.config:149,194,199`</span> |
| `--sniffles_time` | `"12h"` | same | <span class="src">`nextflow.config:151,195,200`</span> |
| `--svim_asm_threads` | `1` | `svim_asm`, `truvari_merge` | <span class="src">`nextflow.config:155,203,208`</span> |
| `--svim_asm_memory` | `null` | same | <span class="src">`nextflow.config:154,204,209`</span> |
| `--svim_asm_time` | `"12h"` | same | <span class="src">`nextflow.config:156,205,210`</span> |
| `--pav_memory` | `"120G"` | `pav_asm` (its `cpus` is the literal `32` unless `--cores` is set) | <span class="src">`nextflow.config:152,340-341`</span> |
| `--pav_time` | `"12h"` | `pav_asm` | <span class="src">`nextflow.config:153,342`</span> |
| `--repeatmasker_threads` | `1` | `split_repeatmask`, `repeatmask_VCF`, `concat_repeatmask` | <span class="src">`nextflow.config:147,213,218,223`</span> |
| `--repeatmasker_memory` | `"10G"` | same | <span class="src">`nextflow.config:146,214,219,224`</span> |
| `--repeatmasker_time` | `"12h"` | same | <span class="src">`nextflow.config:148,215,220,225`</span> |
| `--tsd_memory` | `"10G"` | `tsd_prep`, `tsd_search`, `tsd_report` (one CPU each) | <span class="src">`nextflow.config:157,229,234,239`</span> |
| `--tsd_time` | `"1h"` | same | <span class="src">`nextflow.config:158,230,235,240`</span> |
| `--hervk_annotate_threads` | `1` | `hervk_annotate` | <span class="src">`nextflow.config:128,289`</span> |
| `--hervk_annotate_memory` | `"10G"` | `hervk_annotate` | <span class="src">`nextflow.config:127,290`</span> |
| `--hervk_annotate_time` | `"12h"` | `hervk_annotate` | <span class="src">`nextflow.config:129,291`</span> |
| `--hervk_reconcile_memory` | `"10G"` | `hervk_reconcile` (one CPU) | <span class="src">`nextflow.config:130,295`</span> |
| `--hervk_reconcile_time` | `"1h"` | `hervk_reconcile` | <span class="src">`nextflow.config:131,296`</span> |
| `--pangenie_threads` | `1` | `pangenie_index`, `pangenie` | <span class="src">`nextflow.config:144,243,248`</span> |
| `--pangenie_memory` | `null` | same | <span class="src">`nextflow.config:143,244,249`</span> |
| `--pangenie_time` | `"12h"` | same | <span class="src">`nextflow.config:145,245,250`</span> |
| `--make_graph_threads` | `1` | `make_graph` | <span class="src">`nextflow.config:133,253`</span> |
| `--make_graph_memory` | `"40G"` | `make_graph` | <span class="src">`nextflow.config:132,254`</span> |
| `--make_graph_time` | `"6h"` | `make_graph` | <span class="src">`nextflow.config:134,255`</span> |
| `--graph_align_threads` | `1` | `bam_to_fastq`, `graph_align_reads` | <span class="src">`nextflow.config:125,258,263`</span> |
| `--graph_align_memory` | `null` | same | <span class="src">`nextflow.config:124,259,264`</span> |
| `--graph_align_time` | `"12h"` | same | <span class="src">`nextflow.config:126,260,265`</span> |
...
<span class="src">`nextflow.config:179-181,298-337`</span>

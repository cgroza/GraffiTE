...
|---|---|---|
| `SVs.vcf` | The merged, non-redundant set of insertions and deletions from every caller and sample, sorted, with `SVLEN` recomputed from the alleles and IDs shortened to `<caller id>_<n>`. Missing genotypes are set to `0`. This is the file Stage B annotates. With `--vcf`, it is a copy of the input. | <span class="src">`module/main.nf:184,192,255-257`</span> |
...
<span class="src">`module/main.nf:616-630`, `main.nf:125-134`</span>
...
<span class="src">`module/main.nf:488-504`, `module/main.nf:278-298`</span>
...
| `TSD_summary.txt` | One row per variant searched, the best duplication found and whether it passed. Columns are in [Target site duplications](../background/tsd.md). | <span class="src">`module/main.nf:553`, `bin/TSD_Match_v2.sh:116`</span> |
| `TSD_full_log.txt` | The full search log: both fragments with a ruler, every candidate duplication and the chosen one, per variant. | <span class="src">`module/main.nf:554`, `bin/TSD_Match_v2.sh:126`</span> |
| `hervk_candidates.vcf` | `--human` only. Every `LTR/ERVK` record of `pangenome.vcf` up to `--hervk_max_svlen`, classified, with its discovery genotypes. Wider than the human subset: a locus the `--human` filter split keeps all its members here. | <span class="src">`module/main.nf:326-354`</span> |
...
<span class="src">`bin/vcf_to_pa_tsv.py:4-15,42-53`, `module/main.nf:356-358`</span>

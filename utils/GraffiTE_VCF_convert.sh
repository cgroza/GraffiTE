...
grep -v "#" $VCFin | \
  sed 's/=/\t/g;s/;/\t/g' | \
  cut -f 1-5,11,13,15,19,21,29,31,33,35,37,39,41,43,45,47,49,51 | \
  awk '{gsub(".","& ", $6); print $0}' | \
...
head_end=$(echo -e "SVLEN\tSVTYPE\tCHR2\tEND\tn_hits\tfrags\thit_lenths\thit_names\thit_classes\thit_strands\thit_IDs\ttotal_match_length\ttotal_match_span\tmam_filter_1\tmam_filter2\tTSD")
